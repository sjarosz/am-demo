#!/usr/bin/env bash
# Provision the OAuth2/OIDC custom-script lab (app8) from the artifacts in this
# directory. Invoked by the amster bootstrap container
# (docker/amster/docker-entrypoint.sh) once per stack; idempotent.
#
# ORG-ONLY: provision.py detects the side from AM_COOKIE_DOMAIN and no-ops on the
# com stack. It targets the local AM via AM_SERVER_URL. See provision.py.
set -euo pipefail
...
echo "Provisioning OAuth2/OIDC script lab (${AM_BASE_URL})"
